...
import {
  ProjectPageActions,
  ProjectBackButton,
} from "@/components/projects/ProjectPageActions";
    ...
    <div className="mx-auto max-w-3xl px-4 py-12 sm:px-0 sm:py-16">
      ...
      <div className="mb-8">
        <div className="flex items-start justify-between gap-4">
          ...
          {project.githubUrl && (
            <div className="hidden md:flex md:items-center md:ml-4 flex-shrink-0">
      ...
      {/* Project image */}
      {project.image && (
        <div className="overflow-hidden rounded-xl border border-white/[0.06] bg-white/[0.02] mb-8">
          <Image
            src={project.image}
            alt={project.title}
            width={1200}
      ...
      {/* Summary */}
      <p className="text-sm text-fg-muted sm:text-base leading-relaxed">
        {project.summary}
      </p>
      ...
      <section className="mt-8">
        ...
        <ul className="space-y-3">
          {project.highlightBullets.map((bullet, idx) => (
            <li
              key={idx}
              className="flex items-start gap-3 text-sm text-fg-muted leading-relaxed"
            >
              <span className="flex-shrink-0 h-6 w-6 rounded-full bg-accent/10 text-accent flex items-center justify-center text-xs font-semibold mt-0.5">
                {idx + 1}
              </span>
              <span>{bullet}</span>
            </li>
